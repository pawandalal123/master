        ...
        <div class="detail_box">
       		<div class="list_button_box clearfix">
                ...
                <p class="fr">Sort By:
                               <input type="hidden" class="hidden" value="<?php echo $CategoryName.'/'.$subCategoryName.'/'.$CategoryID.'/'.$subCategoryID;?>"/>
                                    ...
                                    <option selected="" value="<?php  $urlArrayData = $this->common->makeUrlSubCatList($CategoryName , $subCategoryName , $CategoryID , $subCategoryID ,$Getdata, 'date' , 'sort'); echo $urlArrayData['URL'];?>" <?php if(in_array('sort_date' , $urlArrayData['paramArray']['sort'])) echo 'selected=selected'?>>Most Recent Ads</option>
                                    <option value="<?php $urlArrayData = $this->common->makeUrlSubCatList($CategoryName , $subCategoryName , $CategoryID , $subCategoryID , $Getdata,'price' , 'sort'); echo  $urlArrayData['URL'];?>" <?php if(in_array('sort_price' , $urlArrayData['paramArray']['sort'])) echo 'selected=selected'?>>Price: Low to High</option>
                                    <option value="<?php $urlArrayData = $this->common->makeUrlSubCatList($CategoryName , $subCategoryName , $CategoryID , $subCategoryID ,$Getdata, 'pricedesc' , 'sort');  echo  $urlArrayData['URL'];?>" <?php if(in_array('sort_pricedesc' , $urlArrayData['paramArray']['sort'])) echo 'selected=selected'?>>Price: High to Low</option>
                                    </select>
                </p>
            </div>
			...
			else
			{
				...
				?>
                	...
                	<div class="list_view_img"><a href="<?php echo SITE_URL;?>productdetail/<?php echo $this->common->cleanURL($Listingdata->title).'/'.$Listingdata->product_id?>"><img title="<?php echo substr($Listingdata->title,0,30);?>" src="<?php echo $path;?>" style="height:90px; width:130px;"></a></div>
                	<div class="list_view_hd"><h3><a href="<?php echo SITE_URL;?>productdetail/<?php echo str_replace(' ','-',$Listingdata->title).'/'.$Listingdata->product_id?>"><?php  
					 ...
					 echo substr($Listingdata->title,0,30);?></a></h3><p><?php echo $Listingdata->state.'|'.$cityName->city.'|'.$categoryNmae.'-'.$subcategoryNmae;?></p></div>
