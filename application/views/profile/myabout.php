	  ...
	  else
	  {
		  ...
		  ?>
                  ...
                  <div class="info_details">
                    	<h3><?php 
						$headcondition=array("heading_id"=>$Contentdata->feature_title);
						$headingName=$this->common->headingNameDisplay($headcondition);
						echo $headingName->heading_name;?></h3>
...
<div class="uplod_t"><a>Upload a Photo</a></div>
  <input type="file" name="logo" class="getimage" id="getimageID<?php echo $Contentdata->feature_id;?>" onchange="readURLChangeImage(this)">
                ...
                <div class="about_form">
            <fieldset class="mrgn-b-20">
            <input type="hidden" name="featureid" value="<?php echo $Contentdata->feature_id?>">
            <label class="fnt-bold fnt-sz10" for="loginName">Company Name</label>
            <select name="company_id"  class="a_f rf" style="width:276px" id="select1" >
 <option value="" >[SELCET]</option>
 <?php
 ...
 ?>
			...
			foreach($headingOptionList as  $headingList)
			{
				if($Contentdata->feature_title==$headingList->heading_id)
				{
					echo '<option value="'.$headingList->heading_id.'" selected="selected">'.$headingList->heading_name.'</option>';
				}
				else
				{
					echo '<option value="'.$headingList->heading_id.'">'.$headingList->heading_name.'</option>';
			...
			?>
                 ...
                 <div class="clear"></div>
                    <label class="fnt-bold fnt-sz10">Product Description</label>
            <textarea id="textarea<?php echo $Contentdata->feature_id?>" name="descriptionupdate" ><?php echo $Contentdata->feature_description;?></textarea>
            <?php echo form_error('descriptionupdate');?>

<script language="javascript1.2">
  generate_wysiwyg('textarea<?php echo $Contentdata->feature_id?>');
</script>
                 <div class="clear"></div>
                <input name="update" value="Update Product" class="btn btn-blue" type="submit" />
                <input name="discard" value="Discard" class="btn btn-blue hideupdate" id="<?php echo $Contentdata->feature_id;?>" type="button" />
                </form>
