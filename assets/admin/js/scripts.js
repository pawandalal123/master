...
jQuery(document).ready(function () {
    jQuery(".navigation ul li .dropdownWrap .navbannerurl").each(function () {
        jQuery(this).attr({
            "rel": jQuery(this).html()
        })
    });
    jQuery(".navigation > ul > li").hover(function () {
        jQuery(".dropdownWrap").hide();
        jQuery(".navigation  ul  li").removeClass("nextlinav");
        var imageurl = jQuery(this).find(".dropdownWrap").find(".innerNavs").eq(0).find(".navbannerurl").attr("rel");
        var navbannerimg = "<img src='" + imageurl + "'/>";
        jQuery(this).find(".dropdownWrap").find(".innerNavs").eq(0).find(".navbannerurl").html(navbannerimg);
        if (jQuery(this).hasClass("last")) {
            jQuery(this).addClass("lastselected")
        } else {
            if (!(jQuery(this).hasClass("select"))) {
                jQuery(this).next("li").addClass("nextlinav");
                jQuery(this).addClass("selected")
            }
        }
        jQuery(this).find("img.pointer").show();
        jQuery(this).find(".dropdownWrap").show();
        var _lhsheight = jQuery(this).find(".lhssection").outerHeight();
        jQuery(this).find(".rhssection").css("min-Height", _lhsheight);
        jQuery(this).find(".rhssection").children(".innerNavs").css("min-Height", _lhsheight);
        jQuery("#overlay").hide();
        if (jQuery(this).find(".dropdownWrap").is(":visible")) {
            jQuery("#overlay").hide()
        }
    }, function () {
        jQuery(this).removeClass("selected");
        jQuery(this).removeClass("lastselected");
        jQuery(".navigation  ul  li").removeClass("nextlinav");
        if (!(jQuery(this).hasClass("select"))) {
            jQuery(this).find("img.pointer").hide()
        }
        jQuery(".dropdownWrap .lhssection ul li").removeClass("selected");
        jQuery(".dropdownWrap .rhssection .innerNavs").hide();
        jQuery(".dropdownWrap .rhssection").each(function () {
            jQuery(this).find(".innerNavs").eq(0).show()
        });
        jQuery(".dropdownWrap .lhssection").each(function () {
            jQuery(this).find("li").eq(0).addClass("selected")
        });
        jQuery(this).find(".dropdownWrap").hide();
        jQuery("#overlay").hide()
    });
    jQuery(".dropdownWrap .lhssection ul li").hover(function () {
        var _index = jQuery(this).index();
        jQuery(this).addClass("selected").siblings("li").removeClass("selected");
        _rhsnav = jQuery(this).parents(".lhssection").next(".rhssection").find(".innerNavs");
        _rhsnav.css({
            "width": 0
        });
        _rhsnav.hide();
    ...
    jQuery(".dropdownWrap .rhssection").hover(function () {
        var _rhsindex = jQuery(this).find(".innerNavs:visible").index();
        jQuery(this).prev(".lhssection").find("li").eq(_rhsindex).addClass("selected")
    })
});
